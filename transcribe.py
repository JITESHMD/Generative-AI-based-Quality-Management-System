...
def calculate_customer_satisfaction(text):
    ...
    return satisfaction, sentiment_score
...
def extract_resolution_time(conversation_text):
    ...
    if match:
        ...
        hours = int(time_to_read / 60)
        minutes = int(time_to_read % 60)
        seconds = int((time_to_read * 60) % 60)
        resolution_time = "{:02d}:{:02d}:{:02d}".format(hours, minutes, seconds)
        return resolution_time
...
def calculate_compliance_adherence_rate(conversation_text):
    ...
    return compliance_adherence_rate
...
##################################################################
            ...
            # st.write(handling_time)
            # st.write(call_resolve)
            # st.write(statisfaction)
            # st.write(abodant_rate)
            st.write("Handling Time:", handling_time)
            st.write("First Call Resolution:", call_resolve)
            st.write("Customer Satisfaction:", statisfaction)
            st.write("Abandonment Rate:", abodant_rate)
            st.write("Call Resolution:", call_resolution)
            st.write("Transfer Rate:", transfer_rate)
            st.write("Average Speed of Answer (ASA):", average_sa)
            st.write("Call Rating:", call_rating)
###########################################################
            st.write("Service Level:", service_level)
            st.write("Resolution Time:", resolution_time)
            st.write("Customer Effort Score (CES):", cal_ces)
            st.write("Customer Retention:", customer_retention)
            st.write("Compliance Adherence Rate:", compliance_adherence_rate)
...
# def perform_inference(text, prompt):
...
#     prompt = '''
#         Features:
#         1) Problem they are facing
#         2) Sentiment analysis for the text with the score
#         INFERENCE FEATURE IN THE TEXT
#             '''
...
#             for trans in all_transcripts:
#                 #st.write(trans)
#                 inference_result = perform_inference(trans, prompt)
#                 all_probelm.append(inference_result)
            
#             st.write(all_probelm)
...
# def extract_problem_sentiment(response_text):
#     problem_pattern = r'Problem= (.+?)(?:\n|$)'
#     sentiment_pattern = r'Sentiment= (.+?)(?:\n|$)'

#     problem_match = re.search(problem_pattern, response_text)
#     sentiment_match = re.search(sentiment_pattern, response_text)
...
#     prompt = '''
#         Features:
#         1) Problem they are facing in the whole text
#         2) Sentiment analysis for the text with the score for whole text
#         INFERENCE FEATURE IN THE TEXT
#         iam need ouput like
#         Problem=''
#         Sentimental=''
...
#             st.write("Transcriptions:")
#             for trans in all_transcripts:
#                 inference_result = generate_response(trans[0], prompt)  # Assuming trans is a list of strings
#                 problem, sentiment = extract_problem_sentiment(inference_result)
#                 st.write("Problem:")
#                 st.write(problem)
#                 st.write("Sentiment:")
#                 st.write(sentiment)
